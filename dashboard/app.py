...
DISPOSITION_OPTIONS = ["IGNORED", "RISK_ACCEPTED", "POSTPONED"]
...
# --------------------------------------------------------------------------
def resolve_finding(finding_id, disposition, note, username):
    run_query(
        """
        ...
        WHERE id = %s
        """,
        (disposition, note, username, finding_id), fetch=False,
    )
    invalidate_dashboard_cache()
...
def bulk_resolve_deployment(deployment_id, disposition, note, username):
    run_query(
        """
        ...
        """,
        (disposition, note, username, deployment_id), fetch=False,
    )
    invalidate_dashboard_cache()
...
def findings_to_csv(findings, dep_id=None):
    ...
    writer.writerow(["deployment_id", "tool", "severity", "title", "status", "cve", "package_name",
                      "installed_version", "file_path", "line_number", "resolved_by", "resolved_at", "resolution_note"])
    for f in findings:
        writer.writerow([
            ...
            f.get("resolved_by"), f.get("resolved_at"), f.get("resolution_note"),
        ])
    return buf.getvalue()
...
def render_finding_row(f):
    ...
    label = f"[{f['tool']}] {f['title'] or '(untitled)'}"
    with st.expander(label):
        st.markdown(
            f'{pill(sev, color_for(sev))} &nbsp;&nbsp; {pill(status.replace("_", " "), color_for(status))}',
            unsafe_allow_html=True,
        )
        st.write("")
        ...
        if status == "OPEN":
            ...
            disposition = c1.selectbox("Disposition", DISPOSITION_OPTIONS, key=f"disp_{f['id']}")
            note = c2.text_input("Note (optional)", key=f"note_{f['id']}")
            if c3.button("Apply", key=f"apply_{f['id']}", use_container_width=True):
                resolve_finding(f["id"], disposition, note, st.session_state.username)
...
def render_deployment_card(dep):
    ...
    if dep["status"] == "DEPLOYED":
        ...
        action_cols[0].button(
            ...
            help="All findings must be Ignored / Risk Accepted / Postponed first.",
        )
    ...
    if open_count > 1:
        with st.expander(f"Bulk-apply to all {open_count} open findings", icon=":material/checklist:"):
            ...
            bulk_disp = bc1.selectbox("Disposition", DISPOSITION_OPTIONS, key=f"bulk_disp_{dep['id']}")
            bulk_note = bc2.text_input("Note (optional)", key=f"bulk_note_{dep['id']}")
            if bc3.button("Apply to all", key=f"bulk_apply_{dep['id']}", use_container_width=True):
                bulk_resolve_deployment(dep["id"], bulk_disp, bulk_note, st.session_state.username)
